...
@interface MXHSendSupply ()<UITextViewDelegate>
{
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithBackgroundImage:@"return.png" title:@"返回" target:self action:@selector(backSupply)];
    
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithIcon:nil title:@"发布" target:self action:@selector(sendSupply)];
    
    self.view.backgroundColor = UIColorFromRGB2(245, 248, 246);
    
    [self addUISendSupply];
}
...
-(void)textViewDidChange:(UITextView *)textView
{
    ...
    _statusLabel.text = [NSString stringWithFormat:@"%ld/100",(long)number];
}
...
-(void) sendSupply {
    ...
    info = @{@"content":value};
    [MXHSupplyTool supplyWithSendInfo:info success:^(NSArray *supply) {
        for (NSString *s in supply) {
