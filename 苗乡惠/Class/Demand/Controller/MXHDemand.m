...
- (void)viewDidLoad
{
    ...
    self.title = @"求购";
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithIcon:@"publish.png" title:nil target:self action:@selector(pushPublishDemand)];
...
- (void)headerRereshing
{
    ...
    [MXHDemandTool demandWithSinceId:(long long)first maxId:0 success:^(NSArray *demand) {
        NSMutableArray *newFrames = [NSMutableArray array];
        for (MXHInfo *s in demand) {
        ...
        // 5.顶部展示最新微博的数目
        [self showNewDemandCount:demand.count];
...
- (void)footerRereshing
{
    ...
    MyLog(@"%lld",last);
    [MXHDemandTool demandWithSinceId:0 maxId:last success:^(NSArray *demand) {
...
#pragma mark - 发布供应
-(void)pushPublishDemand{
    MXHSendDemand *send = [[MXHSendDemand alloc] init];
